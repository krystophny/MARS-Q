%function MacMain
%% setup for the circular case

global Mac
global SDIR FEEDI KFEED RIPPLE_MAX DISP_MAX

format short e

Mac.SS = 'b-';
Mac.plot_RZ = 0;
Mac.plot_shape = 0;
Mac.plot_RZM= 0;
Mac.plot_JACOB0 = 0;
Mac.checkRZ = 0;  % compare RZ from Fourier series with RZ from real space
Mac.checkJACOB = 0; % compare jacobian computed from RZ with that computed in CHEASE
Mac.plot_JACOB = 0;

Mac.plot_orbit = 0;

Mac.RunEQ = 1;
Mac.plot_EQ = 10;

Mac.RunJ = 0;
Mac.NRES = 0;
Mac.plot_JM = 1;
Mac.plot_JM_EQ = 2;
Mac.plot_JM_KD = 5;
Mac.spline_J1 = 0;
Mac.plot_JP = 0;
Mac.plot_JW = 0;
Mac.plot_JW2 = 3;
Mac.plot_Js = 0;
Mac.plot_Jphi = 0;  %plot Jphi along outboard major radius
Mac.plot_J3U = 0;
Mac.plot_J2J3= 0;
Mac.plot_Jpara = 3;
Mac.save_FEEDJ = 0;
Mac.read_FEEDJ = 0;
Mac.plot_FEEDJ = 0;
Mac.plot_shell = 0;
Mac.plot_shdiv = 0;
Mac.DivJ  = 0;
Mac.plot_DivJ = 0;
Mac.ReduceNs = 1;  %reduce radial points when output J on Gaussian points

Mac.RunB = 0;
Mac.RZmap = 0;
Mac.plot_BM = 0;
Mac.plot_Bn = 0;
Mac.spline_B23 = 0;  %preferred option is 0
Mac.plot_B  = 0;
Mac.plot_B_2D = 0;
Mac.plot_BR = 0;
Mac.plot_BW = 0;     %plot 2D B-field at wall
Mac.plot_BC = 0;     %plot real-space Bwr,Bwz,Bwphi along physical theta
Mac.plot_Bs = 0;     %plot MARS-F-space B1,B2,B3 along chi at wall
Mac.plot_BWP = 4;    %plot real-space Bwn,Bwt,Bwphi along physical theta 
Mac.plot_BWM = 0;    %plot harmonics of Bwn, Bwt, Bwphi for physical theta
Mac.plot_BWC = 0;    
Mac.plot_BWR = 0;    %plot (BR,BZ) along Mac.rw-surface
Mac.save_BNORM = 0;   %1 <--> BOVACU01; 2<-->BOVACU02; 3<-->BOVACU03; 4<-->BOVACU04; 
Mac.check_BM = 0;    %compare BM from MARS-F and BM from CARIDDI
Mac.plot_BMcheck = 0;
Mac.CNORM = 1.0;
Mac.DivB  = 0;
Mac.plot_DivB = 0;
Mac.plot_JW3  = 0;
Mac.Kratsurf  = 1;
Mac.Iratsurf  = [];
Mac.FullSol   = 0;
Mac.FootPrint = 0;
Mac.plot_Tjxb = 0;

Mac.RunEF   = 0;  %1: read error field at one surface in vacuum, for generating equivalent surface current
                  %2: read error field at one surface inside plasma, for testing 
		  %3: read vacuum field data at MacSurfS from PROB_G 
Mac.plot_EF = 0;

Mac.RunV    = 0;
Mac.plot_VM = 3;
Mac.plot_V  = 0;
Mac.plot_Vn = 4;
Mac.plot_VS = 0;  %for SVD

Mac.RunP    = 0;
Mac.plot_PM = 0;
Mac.plot_PMM= 0;
Mac.plot_Pfluid = 5;
Mac.plot_Pperp = 0;
Mac.plot_Ppara = 0;
Mac.plot_DPHI = 0; 

Mac.plot_coil = 0;
Mac.NGauss = 2;      % order of Gauss quadrature integration
Mac.Nchi   = 513;    % number of points along poloidal angle 'chi'
Mac.Nm2    = 100;     % number of poloidal harmonics for RZ-construction
Mac.Nm0    = [];     % number of poloidal harmonics for equilibrium
Mac.Nm1    = [];     % number of poloidal harmonics for stability
Mac.Ns1    = [];     % number of radial points in plasmas
Mac.Ns2    = [];     % number of radial points in vacuum
Mac.Ns     = [];     % = Mac.Ns1 + MacNs2;
Mac.chi    = [];     % poloidal coordinate 'chi'
Mac.phi    = [];     % toroidal coordinate 'phi'
Mac.s      = [];     % radial coordinate 's'
Mac.rw     = [];     % radial position of flux surfaces to be saved
Mac.Mm     = [];     % array of poloidal harmonics for stability
Mac.n      = [];     % toroidal mode number
Mac.Norm   = 1;      % Apply normalization
Mac.R0EXP  = 2.0;    % active if Mac.Norm=0 
Mac.B0EXP  = 1.0;    % active if Mac.Norm=0 
Mac.HWEXP  = 0.01;   % wall thickness [m]
Mac.coil   = [1.24 0.1334 0.2584; 1.24 -0.2530 -0.123];     
           % coils position defined as [rc chi_1 chi_2]
Mac.resetCoil = 0;
%Mac.coilN  = [2.184 1.012 2.394 0.504; 2.394 -0.504 2.184 -1.012];     
             % D3D I-coils: [R1 Z1 R2 Z2;...]
%Mac.coilN  = [3.23 0.80 3.23 -0.80];     
             % D3D C-coils
Mac.coilN   = [5.13 5.92 6.22 3.50; 6.60 2.21 6.60 -2.21; 6.22 -3.50 5.13 -5.92]; 
             % STEP RWM/ELM control coils  
%Mac.rw     = [0.9294];
%Mac.rw      = [ 0.88770577E+00];
%Mac.rw     = [1.0 1.0523];
Mac.rw     = [1.0 1.1939];  %NSTX132543
%Mac.rw     = [1.01];
%Mac.rw     = [4.7576];
%Mac.rw     = [1.1581]; %ITER 10MA
%Mac.rw     = [2.656];  %RE_KINK_NEW
%Mac.rw     = [2.8349]; %RE_KINK=D3D177040
%Mac.rw     = [1.0063]; %RE_KINK=D3D177040
%Mac.rw      = [2.211];  %JET95135
%Mac.rw      = [1.006];  %JET95135
%Mac.rw      = [4.758];  %D3D165370
%Mac.rw      = [1.0088];  %D3D165370
%Mac.rw     =  [1.0 1.207];
%Mac.rw     =  [1.0 1.178];
%Mac.rw     = [0.9294 1 1.0385 1.1922];
Mac.rs     = Mac.rw(1); % the position of equivalent sheet current from CARIDDI
Mac.cut_RZ = -1;
Mac.plot_thickness = 0;
Mac.CHIS   = 0;
Mac.mm_plot =-9:155;
Mac.edge = 1;
Mac.core = 0.21;

data3 = [
%shot# B0EXP(T) R0EXP(M) R/a    ne0(m^-3)  Ti0(eV)   Te0(eV)   w0(rad/s)  tauA(s)    OMEGACI0   1/S        w0/wA      ALPHAP    
 10001 5.3      6.2      3.1022 8.2506e+19 3.0659e+4 2.9157e+4 1.1894e+4  7.7025e-07 1.5642e+02 7.8635e-11 9.1614e-03 5.1256e-01
 10005 2.65     6.2      3.1163 1.5826e+19 3.4715e+3 2.0510e+4 1.0005e+4  6.7469e-07 6.8506e+01 7.3869e-11 6.7503e-03 1.4476e-01
 10006 2.65     6.2      3.2644 1.5766e+19 3.6662e+3 1.4396e+4 9.9748e+3  6.7341e-07 6.8376e+01 1.3758e-10 6.7171e-03 2.0298e-01
 10007 2.65     6.2      3.2743 2.1570e+19 3.6201e+3 1.9426e+4 9.9582e+3  7.8767e-07 7.9978e+01 1.0328e-10 7.8438e-03 1.5708e-01
 10008 5.3      6.2      3.1000 6.7940e+19 3.6030e+4 3.4350e+4 5.6306e+4  6.5542e-07 1.3310e+02 4.6732e-11 3.6904e-02 5.1194e-01
];

Mac.kSVD = 0;
Mac.kRAW = 0;
kd       = 5;
Mac.DEN  = 1.03e+20;
Mac.TI   = 1.2216e+3;
Mac.TE   = 0.9929e+3;


%afac = 1.0/1.4290e+04; phas = 0.5*pi;  %fluid
%afac = 1.0/4.9035e+03; phas =-4.4287e-1*pi; %hybrid
%afac  = 1.1995e-4; %IP-RWM, ITER-10MA
%afac  = 1.1341e-4; %RP-RWM, ITER-10MA
%afac   = 4.9510e-05; phas =-1.5668;     %RE_KINK_NEW, fluid
%afac   = 6.2798e-05; phas =-4.3269e-01; %RE_KINK_NEW, hybrid
%afac   = 4.7437e-2; phas=0.0; %JET95135
%afac   = 3.6583e-2; phas=0.0; %D3D165370
%afac = 6.2150e-6; phas = 0.0;
%afac = 5.8589E-02; phas = 0.0;
%afac = 1.2239E-05; phas=0;
%afac  = 1e+0; phas=0;
%afac   = 1.0557e-1; phas=0.0; %D3D177040, maxdB/B0=10%
%afac  = 4.2765e-1;  phas=0;   %D3D184602, maxdB/B0=10%
%afac  = 3.0427e-1;  phas=0;   %D3D185604, maxdB/B0=10%
%afac  = 5.4249e-2;  phas=0;   %ITER-DINA_68ms, maxdB/B0=10%
%afac  = 1.1011e-1;  phas=0;   %ITER-DINA_76ms, maxdB/B0=10%
%afac  = 5.8446e-2;  phas=0;   %ITER-DINA_86ms, maxdB/B0=10%
%afac  = 2.5332e-2; phas=0;  %MAST-U45163, maxdB=100G
afac  = 1.0e-0; phas=0; 
%afac = 5.7660e-04; phas=0; %MAST-U46943_Case1_NVG_OLD
%afac = 5.7587e-02; phas=0; %MAST-U46943_Case1_NVG_NEW
%afac = 6.3710e-04; phas=0; %MAST-U46943_Case2
%afac  = 1.5886e-1; phas=0; %D3D177028/RootAR
%afac  = 4.4229e-1; phas=0; %D3D177028/RootDR

SCASE = '.OUT';
%SCASE = '_Q0.95';
%SCASE = '_Q1.05';
%SCASE = '_RRP';
%SCASE = '_ISP';
%SCASE = '_120';
%SCASE = '_U_KIN_INCDPHI1';
%SCASE = '_EP_DPHI';

%SDIR5  = 'U/';
%SDIR5  = 'RES_ALL_NDB1/';
%SDIR5 = 'M48/';
%SDIR5 = 'RootC/';
SDIR5 = 'n18/';

Mac.ALPHA   = 1.0;                     
Mac.ShellCurrNorm = 1.0;
Mac.BNORM = afac*exp(phas*i);
Mac.JNORM = afac*exp(phas*i);
Mac.PNORM = afac*exp(phas*i)/2; %KSTAR TCI data = n_e*deltaTe => factor of 2 here 
Mac.VNORM = afac*exp(phas*i);
Mac.FNORM = afac*exp(phas*i);  %normalization for electrostatic potential

%SDIR = '/home/liuy/Work/D3D161015/Work/';
%SDIR = '/home/liuy/Work/D3D166439/Data/';
%SDIR = '/cscratch/liuy/WorkCFETRc/';
%SDIR = '/cscratch/liuy/RE_ITER/86ms_P100_S+1/';
%SDIR = '/home/liuy/Work/RE_ITER/Data_HALO/76ms/';
%SDIR = '/cscratch/liuy/WorkAVALANCHE/';
%SDIR = '/cscratch/liuy/WorkD3D174446/';
%SDIR = '/home/liuy/Work/D3D165370/ReIK/Common/';
%SDIR = '/home/liuy/Work/RE_KINK/Data_REK/Common/';
%SDIR = '/home/liuy/Work/ITER/Data_EP/10MA/Common/';
%SDIR = '/home/liuy/Work/ITER/Data_RE/Common/';
%SDIR = '/home/liuy/Work/RE_IK/Data/';
%SDIR = '/cscratch/liuy/WorkRE_KINK_NEW/';
%SDIR = '/cscratch/liuy/WorkTEMP/';
%SDIR = '/home/liuy/Work/RE_KINK_NEW/Data/';
%SDIR = '/home/liuy/Work/JET95135/Data/';
%SDIR = '/cscratch/liuy/WorkRE_KINK/';
%SDIR = '/home/liuy/Work/RE_COMPASS/Data/n3/';
%SDIR = '/cscratch/liuy/WorkRE_ITER/86ms/';
%SDIR = '/cscratch/liuy/WorkITERb/';
%SDIR = '/home/liuy/Work/ITER/Data_EP/15MAQ10/Poincare/n3_3row_trap/RMP/';
%SDIR = '/home/liuy/Work/ITER/Data_Marina_12.5MA/';
%SDIR = '/home/liuy/Work/ITER/Data_EP/10MA/Common/';
%SDIR = '/home/liuy/Work/D3D_IK/EIGENFUN/';
%SDIR = '/cscratch/liuy/IR_D/Database/DIII-D/157376/3D/n3/';
%SDIR = '/cscratch/liuy/IR_D/Database/MAST/25075/3D/n1/';
%SDIR = '/cscratch/liuy/IR_D/Database/AUG/31131.6400/3D/n1/';
%SDIR = '/cscratch/liuy/IR_D/Database/ITER/131025.0002/3D/n1/';
%SDIR = '/home/liuy/Work/RE_KINK_NEW/Data_DiffConv/Common/';
%SDIR = '/home/liuy/Work/RE-D3D/Data_184602/';
%SDIR = '/home/liuy/Work/RE-D3D/Data_185604/';
%SDIR = '/cscratch/liuy/RE_KINK/Single/';
%SDIR = '/home/liuy/Work/RE_transport/177040/Common/';
%SDIR = '/home/liuy/Work/RE_transport/184602/Common/';
%SDIR = '/home/liuy/Work/RE_transport/185604/Common/';
%SDIR = '/home/liuy/Work/RE_transport/68ms/Common/';
%SDIR = '/home/liuy/Work/RE_transport/76ms/Common/';
%SDIR = '/home/liuy/Work/RE_transport/86ms/Common/';
%SDIR = '/cscratch/liuy/147178/Work/';
%SDIR = '/cscratch/liuy/153485/Work/';
%SDIR = '/cscratch/liuy/D3D136642_VDE/';
%SDIR = '/cscratch/liuy/D3D088806_VDE/';
%SDIR = '/home/liuy/Work/KSTAR/Data/29466/';
%SDIR = '/cscratch/liuy/WorkMAST/';
%SDIR = '/cscratch/liuy/D3D191366_RE_DiMES/';
%SDIR = '/home/liuy/Work/D3D184003/Data/';
%SDIR = '/home/liuy/Work/MAST-U45163/Data/';
%SDIR = '/cscratch/liuy/WorkIR_D/BU/';
%SDIR = '/cscratch/liuy/Database_More/EAST/69635/3D/n1/';
%SDIR = '/cscratch/liuy/YangXu/Pp0_11/';
%SDIR = '/cscratch/liuy/YangXu/Pp0_11M/';
%SDIR = '/cscratch/liuy/YangXu/Pp0_015/';
%SDIR = '/cscratch/liuy/YangXu/Pp0_015M/';
%SDIR = '/home/liuy/Work/KSTAR/Data/30306.4850/';
%SDIR = '/home/liuy/Work/KSTAR/Data/30306.4850_gpec/';
%SDIR = '/home/liuy/Work/KSTAR/Data/30306.7850/';
%SDIR = '/home/liuy/Work/D3D176440/Data/';
%SDIR = '/home/liuy/Work/D3D194202/Data/';
%SDIR = '/cscratch/liuy/Cote/';
%SDIR = '/home/liuy/Work/KSTAR/Data/30306.7850_gpec/';
%SDIR = '/home/liuy/Work/STEP-P/Case3/Data/';
%SDIR = '/cscratch/liuy/STEP-P/Case3/';
%SDIR = '/cscratch/liuy/STEP-P/Temp3/';
%SDIR = '/home/liuy/Work/ITER_RIPPLE/Data/10MA/';
%SDIR = '/home/liuy/Work/ITER_RIPPLE/Data/5MA_GAP5/';
%SDIR = '/home/liuy/Work/EAST94048/Data/';
%SDIR = '/home/liuy/Work/Xpoint/Data/';
%SDIR = '/home/liuy/Work/Solovev/Data/';
%SDIR = '/home/liuy/Work/MAST_RWM/Data/27112/RFA_NEW/';
%SDIR = '/cscratch/liuy/MAST-U/';
%SDIR = '/home/liuy/Work/MAST-U/46943/Data/';
%SDIR = '/home/liuy/Work/MAST-U/45272/Data/RMP_n2/';
%SDIR = '/cscratch/liuy/Temp/';
%SDIR = '/cscratch/liuy/D3D_QH_CETOP/Temp/';
%SDIR = '/home/liuy/Work/KSTAR/Data/30306.7850_gpec/FIDIST_5D/2D/';
%SDIR = '/cscratch/liuy/NSTX132543/';
%SDIR = '/home/liuy/Work/DIII-D/D3D177028/Data/';
%SDIR = '/cscratch/liuy/ZhangHY/EAST52340_3150/';
%SDIR = '/home/liuy/Work/ITER/ITER_RE/DataAE_HALO/68ms/';
%SDIR = '/cscratch/liuy/QinPei/USN_n1_20degree_pls/Rmars/';
%SDIR = '/cscratch/liuy/MAST-U/';
%SDIR = '/home/liuy/Work/DIII-D/D3D_QH_CETOP/Data/163519.2650/n2/';
SDIR = '/home/liuy/Work/NSTX/NSTX132543/Data/';

%eval(['load ' SDIR 'FEEDI']);
%KFEED = 19;


if Mac.checkRZ, Mac.plot=1; end
Mac.chi = linspace(-pi,pi,Mac.Nchi);
Mac.phi = linspace(0,2*pi,101); Mac.phi = Mac.phi(:);

%Mac.chi = Mac.chi(1:end-1);
%Mac.Nchi = Mac.Nchi-1;

% RZ coordinates
[RM,ZM] = MacReadRMZM([SDIR SDIR5 'RMZM_F.OUT']);
[R,Z] = MacGetRZ(RM,ZM);
if Mac.checkRZ, MacCheckRZ('../TestCirc/RMZM_R'), end

if Mac.RunEQ
   %MacReadPROFEQ([SDIR 'PROFEQ_CORRECT']);
   MacReadPROFEQ([SDIR 'PROFEQ.OUT']);
end

if Mac.plot_orbit > 0
   MacPlotOrbit([SDIR 'ORBIT.OUT']);
end

% normalization for B-field
fac_c = 0.3852;      %=I_n/I_0
I0EXP = 1.0e+3*fac_c;      %[kA] vacuum shot
mu0   = 4e-7*pi;
fac   = mu0/Mac.R0EXP;
FEEDI = 1.0;

%Mac.BNORM  = Mac.BNORM*fac*I0EXP/FEEDI*1e+4;  %in Gauss/kA
%Mac.VNORM  = Mac.BNORM*Mac.R0EXP/Mac.B0EXP/1e+4; %in meter/kA
%Mac.JNORM  = Mac.BNORM*(Mac.B0EXP/Mac.R0EXP/4.0e-7/pi)/Mac.B0EXP/1e+4/1e+3; %in kA/kA

Mac.BNORM  = Mac.BNORM*Mac.B0EXP*1e+4;  %in [Gauss]
Mac.VNORM  = Mac.VNORM*Mac.R0EXP*1e+3; %in [mm]
Mac.JNORM  = Mac.JNORM*Mac.B0EXP/Mac.R0EXP/(4e-7*pi)/1e+6; %[MA/m^2]
Mac.PNORM  = Mac.PNORM*Mac.B0EXP^2/(4e-7*pi)/1e+3;  %[kPascal] 
Mac.FNORM  = Mac.FNORM*Mac.B0EXP^2/(4e-7*pi)/(Mac.DEN*1.6021917e-19); %[V]

if Mac.RunEF>0
   %Mac.BNORM   = Mac.B0EXP*1e+4*(4.8881e-04+1.4664e-03i)/1.05;
   %Mac.BNORM   = Mac.B0EXP*1e+4*(9.7775e-04+9.7775e-04i);
   %Mac.BNORM   = Mac.B0EXP*1e+4;
   Mac.BNORM   = 1.0;
end

% RZ coordinates for the coils
if Mac.resetCoil, MacResetCoil(R,Z), end
if length(Mac.coil)>0, [Rc,Zc] = MacGetRZcoil(RM,ZM); end

% unit vectors e_s and e_chi and jacobian
[dRds,dZds,dRdchi,dZdchi,jacobian] = MacGetUnitVec(R,Z);
if Mac.checkJACOB, MacCheckJacob([SDIR 'JACOBIAN_fix'],jacobian), end

%read in error field data
if Mac.RunEF==1
   %MacGetB1EF([SDIR '10738_surf_ef_n=1.dat'],R,Z,dRdchi,dZdchi);
   %MacGetB1EF([SDIR '28002_surf_ef_n=1_10738ESC.dat'],R,Z,dRdchi,dZdchi);
   %MacGetB1EF([SDIR '10738_surf_ef_n=2.dat'],R,Z,dRdchi,dZdchi);
   %MacGetB1EF([SDIR '26128_surf_ef_n=1.dat'],R,Z,dRdchi,dZdchi);
    MacGetB1EF([SDIR '26128_surf_efcc_n=1.dat'],R,Z,dRdchi,dZdchi);
elseif Mac.RunEF==2
   %MacGetB1EF([SDIR 'decompose_surf_q=2_intrinsic_n=1.dat'],R,Z,dRdchi,dZdchi);
   %MacGetB1EF([SDIR 'decompose_surf_n=1_efcc_2_8_minus_1kat.dat'],R,Z,dRdchi,dZdchi);
   %MacGetB1EF([SDIR 'decompose_surf_n=1_efcc_5_11_minus_1kat.dat'],R,Z,dRdchi,dZdchi);
   MacGetB1EF([SDIR 'decompose_surf_n=1_efcc_2_8_minus_1kat_efcc_5_11_minus_point_5.dat'],R,Z,dRdchi,dZdchi);
elseif Mac.RunEF==3
   MacGetB1EF([SDIR '../PROBE_G/probe_gb.out_c'],R,Z,dRdchi,dZdchi);
end

% perturbed plasma current
if Mac.RunJ
[JM1,JM2,JM3,JMRE] = MacReadJPLASMA([SDIR SDIR5 'JPLASMA.OUT'],R,Z,jacobian);
if Mac.DivJ & Mac.spline_J1==0, MacCheckDivJ(JM1, JM2, JM3); end
if Mac.plot_J3U, MacGetJ3UMAX('../SHOT117327/J3UMAX_RFA_I2P05E0T2e4W8em7'), end
if 1==1
[J1,J2,J3,JRE] = MacGetJ123(JM1,JM2,JM3,JMRE,R,Z);
[Jr, Jz, Jphi] = MacGetJphysC(R,Z,dRds,dZds,dRdchi,dZdchi,jacobian,J1,J2,J3);
MacGetJatGaussQuad(R, Z, Jr, Jz, Jphi);
MacGetJatWall(R, Z, Jr, Jz, Jphi);
MacGetJpara(R,Z,dRds,dZds,dRdchi,dZdchi,jacobian,J1,J2,J3,JRE);
[Jrho, Jchi, Jphi] = MacGetJphysT(R,Z,dRds,dZds,dRdchi,dZdchi,jacobian,J1,J2,J3);
end

% equivalent sheet current from CARIDDI
if length(Mac.rs)==1 & Mac.read_FEEDJ 
[TC, JrC, JzC, JphiC] = MacReadJCARIDDI('../TestCirc/shell_F_iteration_19.asc');
[CHIG,WG,J1G,J2G,J3G] = MacGetJ123G(TC,JrC,JzC,JphiC,R,Z,dRds,dZds,dRdchi,dZdchi,jacobian);
[JM1C,JM2C,JM3C] = MacGetJM123(CHIG,WG,J1G,J2G,J3G);
end

end

% perturbed magnetic field
if Mac.RunB
%MacReadGAMMA('../TestCirc/GAMMA_TMP');
[BM1,BM2,BM3] = MacReadBPLASMA([SDIR SDIR5 'BPLASMA' SCASE]);

if 1==1

[B1,B2,B3,Bn] = MacGetB123(BM1,BM2,BM3,R,Z,dRdchi,dZdchi);
%MacGetBsn(B1,B2);
if Mac.DivB & Mac.spline_B23==0, MacCheckDivB(BM1, BM2, BM3); end
%[Brho, Bchi, Bphi] = MacGetBphysT(R,Z,dRds,dZds,dRdchi,dZdchi,jacobian,B1,B2,B3);
[Br, Bz, Bphi] = MacGetBphysC(R,Z,dRds,dZds,dRdchi,dZdchi,jacobian,B1,B2,B3);
[Bnm,Btm,Bpm]=MacGetBatWall(R, Z, Br, Bz, Bphi);
%MacGetBnWall(R, Z, Br, Bz, Bphi);

if Mac.FootPrint>0, MacGetBforFootPrint(R,Z,Br,Bz); end

if length(Mac.rw) == 1 & Mac.save_BNORM == 1
[TC, BrC, BzC, BphiC] = MacReadBCARIDDI('../TestCirc/MacDataBs_CARIDDI');
[TC, BrC, BzC, BphiC] = MacReadBMARSF('../TestCirc/MacDataBs_MARSF');
[CHIG,WG,B1G,B2G,B3G] = MacGetB123G(TC,BrC,BzC,BphiC,R,Z,dRds,dZds,dRdchi,dZdchi,jacobian);
[BM1C,BM2C,BM3C] = MacGetBM123(CHIG,WG,B1G,B2G,B3G);
if Mac.check_BM, MacCheckBM(BM1,BM2,BM3,-BM1C,-BM2C,-BM3C); end
elseif length(Mac.rw) == 2 & (Mac.save_BNORM == 3|Mac.save_BNORM == 4)
MacSplitBCARIDDI('../TestCirc/MacDataBs_2N'); 
rw_save = Mac.rw;
Mac.rw = rw_save(1);
[TC, BrC, BzC, BphiC] = MacReadBCARIDDI('MacDataBs_TMP1');
[CHIG,WG,B1G,B2G,B3G] = MacGetB123G(TC,BrC,BzC,BphiC,R,Z,dRds,dZds,dRdchi,dZdchi,jacobian);
[BM1C1,BM2C1,BM3C1] = MacGetBM123(CHIG,WG,B1G,B2G,B3G);
Mac.rw = rw_save(2);
[TC, BrC, BzC, BphiC] = MacReadBCARIDDI('MacDataBs_TMP2');
[CHIG,WG,B1G,B2G,B3G] = MacGetB123G(TC,BrC,BzC,BphiC,R,Z,dRds,dZds,dRdchi,dZdchi,jacobian);
[BM1C2,BM2C2,BM3C2] = MacGetBM123(CHIG,WG,B1G,B2G,B3G);
Mac.rw = rw_save;
tmp = BM1C1;
tmp1 = [real(tmp(:)) imag(tmp(:))];
tmp = BM2C2;
tmp2 = [real(tmp(:)) imag(tmp(:))];
if Mac.save_BNORM == 3
  tmp = BM1C1./BM2C2;
elseif Mac.save_BNORM == 4
  tmp = BM1C1/sum(BM2C2);
end
tmp3 = [real(tmp(:)) imag(tmp(:))];
tmp = [tmp1 tmp2 tmp3];
if Mac.save_BNORM == 3
  save BNORM03_CARIDDI tmp3 -ascii -double
elseif Mac.save_BNORM == 4
  save BNORM04_CARIDDI tmp3 -ascii -double
end
end
end
end

% compute toroidal component of the jxb torque density
if Mac.RunJ & Mac.RunB & Mac.spline_B23==0 & Mac.spline_J1==0 
[Tjxb,Ajxb] = MacGetTorqJXB(BM1,BM2,JM1,JM2,jacobian);
res_JXB = Ajxb(end)*Mac.R0EXP^3*Mac.B0EXP^2/4e-7/pi 
end

% perturbed velocity
if Mac.RunV
[VM1,VM2,VM3,DPSIDS,T,VM1N] = MacReadVPLASMA([SDIR SDIR5 'XPLASMA' SCASE]);
Mac.DPSIEQ=DPSIDS;  Mac.TEQ=T;
[V1,V2,V3,Vn] = MacGetV123(VM1,VM2,VM3,VM1N,R,Z,dRds,dZds,dRdchi,dZdchi,jacobian);
[Vr, Vz, Vphi] = MacGetVphys(R,Z,dRds,dZds,dRdchi,dZdchi,jacobian,V1,V2,V3);
%data = load([SDIR 'PROFEQ.OUT']); density = data(:,5);
%den_pump_out   = MacGetDenPumpOut(R,V1,jacobian,density)

[Y,Imid] = max(R(Mac.Ns1,:))
[Y,Ixpt] = min(Z(Mac.Ns1,:))
res_Vn = [max(abs(Vn(end,:))) mean(abs(Vn(end,:))) abs(Vn(end,Imid)) abs(Vn(end,Ixpt))]*Mac.R0EXP*1e+3 

if Mac.plot_Vn>0 & 1==0
   d  = load([SDIR 'PROFDISP.OUT']);
   tt = d(:,1);
   Xn = d(:,2)+d(:,3)*i;
   II = find(tt>pi); tt(II) = tt(II)-2*pi;
   [tt,II] = sort(tt);
   Xn = Xn(II);
   figure(10*Mac.plot_Vn+7)
   %plot(tt*180/pi,abs(Xn)*Mac.R0EXP*1e+3,'r-','LineWidth',2)
   CX = Mac.chi(Ixpt)*180/pi;
   a=axis;
   plot([CX CX],[a(3) a(4)],'k--')
end
end


% perturbed pressure
if Mac.RunP
[PM1,PM2,PM3,PM4,PM5] = MacReadPPLASMA([SDIR SDIR5 'PPLASMA' SCASE]);
[P1,P2,P3] = MacGetP123(PM1,PM2,PM3,PM4,PM5,R,Z);
end

%compare two ways of calculating delta_rho
if 1==0
   %method-I: direct read from DPLASMA.OUT
   d  = load([SDIR 'DPLASMA.OUT']);
   MM = d(1,1); 
   NN = d(1,2);
   d  = d(MM+2:end,:);
   drho1=d(:,1)+d(:,2)*i;
   drho1=reshape(drho1,NN,MM);
   drho1=drho1(1:NN-1,:);
   
   %method-II: drho=-xi^1 dot drho/ds
   d  = load([SDIR 'PROFEQ.OUT']);
   s  = d(:,1);
   rho= d(:,5);
   sx = (s(1:end-1)+s(2:end))/2;
   drhods=diff(rho)./diff(s);
   drho2 =(VM1(1:end-1,:)+VM1(2:end,:))/2;
   for m=1:MM
       drho2(:,m)=-drho2(:,m).*drhods;
   end
 
   %plot and compare
   if 1==0
   figure(98)
   plot(sx,real(drho1),'-'), hold on
   plot(sx,real(drho2),'--'), hold on
   a=axis; axis([0.97 1 a(3) a(4)])

   figure(99)
   plot(sx,imag(drho1),'-'), hold on
   plot(sx,imag(drho2),'--'), hold on
   a=axis; axis([0.97 1 a(3) a(4)])
   end
end


if 1==0
%compare B1 and V1
data = load([SDIR 'PROFEQ_PEST']); 
psip = data(end,12)
F    = data(end,13)
N    = Mac.Ns1;
B1E  = B1(N,:);
V1E  = V1(N,:);
RE   = R(N,:);
JE   = jacobian(N,:);
ntor = -3;
x    = Mac.chi;
xx   = (x(1:end-1)+x(2:end))/2;
yy   = diff(V1E)./diff(x);
zz   = spline(xx,yy,x);

Y1E  = psip*zz + JE*F./RE.^2*i*ntor.*V1E;

figure
plot(x,JE)
figure
plot(x,RE)
figure
plot(x,abs(V1E))
figure
plot(x,abs(zz))

figure
plot(x,real(B1E),'r-','LineWidth',2), hold on,
plot(x,real(Y1E),'b--','LineWidth',2), hold on,

figure
plot(x,imag(B1E),'r-','LineWidth',2), hold on,
plot(x,imag(Y1E),'b--','LineWidth',2), hold on,
end

if 1==0 & (Mac.RunV==1 | Mac.RunP==1)
   run /home/liuy/Work/KSTAR/Work/drawMemo.m
end

if 1==0
   d=load([SDIR 'PROFWE_SAVE.IN']);
   figure(106)
   plot(d(2:end,1),d(2:end,2)/1000,'k-','LineWidth',3)

   plot([0 1],[0 0],'k-')
   sr = Mac.s(Mac.Iratsurf);
   a=axis; 
   for k=1:length(sr)
       plot([sr(k)^2 sr(k)^2],[a(3) a(4)],'k--')
   end
end

GLOBAL_MAX = [RIPPLE_MAX DISP_MAX]
