function MacACADplot(SDIR2,SCOIL,SFLD,SEXB)
%% setup for the circular case
% SCOIL = 'IU' or 'IL'
% SFLD  = ''(TOT),'VAC','PLS'
% SEXB  = ''(fluid flow),'_EXB'
...
Mac.plot_BWP = 6;    %plot real-space Bwn,Bwt,Bwphi along physical theta 
...
AMP = 1.0;
if isstruct(Acad)
   if isfield(Acad,'AMP') AMP=Acad.AMP; end
end

phas = 0.0*pi;
Mac.ALPHA   = 1.0;                     
Mac.ShellCurrNorm = 1.0;
Mac.BNORM = AMP*exp(phas*i);
Mac.JNORM = AMP*exp(phas*i);
Mac.PNORM = AMP*exp(phas*i); 
Mac.VNORM = AMP*exp(phas*i);
...
%SDIR = '/home/liuy/Work/COMPASS/Data/';
...
% RZ coordinates
[RM,ZM] = MacReadRMZM([SDIR 'RMZM_F_EQAC']);
[R,Z] = MacGetRZ(RM,ZM);
...
Mac.rw = Mac.s(Mac.Ns1+Acad.NW-1);
Mac.rs = Mac.rw;
...
if Mac.RunJ
[JM1,JM2,JM3] = MacReadJPLASMA([SDIR 'JPLASMA' SEXB '_' SCOIL '.OUT']);
...
if Mac.plot_Jpara > 0, MacGetJpara(R,Z,dRds,dZds,dRdchi,dZdchi,jacobian,J1,J2,J3), end
...
[BM1,BM2,BM3] = MacReadBPLASMA([SDIR 'BPLASMA' SEXB '_' SCOIL SFLD '.OUT']);
...
% perturbed velocity
if Mac.RunV
[VM1,VM2,VM3,DPSIDS,T] = MacReadVPLASMA([SDIR 'XPLASMA' SEXB '_' SCOIL '.OUT']);
...
% perturbed pressure
if Mac.RunP
[PM1,PM2,PM3] = MacReadPPLASMA([SDIR 'PPLASMA' SEXB '_' SCOIL '.OUT']);
end
