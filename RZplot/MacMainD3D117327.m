...
Mac.plot_BWC= 0;     %plot 2D B-field at wall
...
%Mac.coilN  = [2.184 1.012 2.394 0.504; 2.184 -1.012 2.394 -0.504; 2.477 0 2.477 0];     % I-coil: [R1 Z1 R2 Z2;...]
Mac.coilN  = [2.184 1.012 2.394 0.504; 2.184 -1.012 2.394 -0.504];     % upper I-coil: [R1 Z1 R2 Z2;...]
Mac.rw     = [0.8];
...
SDIR = '/.automount/funsrv1/root/home/yliu/D3D117327/';
...
if Mac.RunEQ
[QEQ] = MacReadPROFEQ([SDIR 'PROFEQ_D3D114094']);
end

% normalization for B-field
I0EXP = 1.0e+3*3/pi;      %[kA] vacuum shot
mu0   = 4e-7*pi;
fac   = mu0/Mac.R0EXP;
%FEEDI = 0.4854; %0.4854(EQAC); %0.3645(PEST);
Mac.BNORM  = fac*I0EXP/FEEDI*1e+4;  %in Gauss
...
if Mac.checkJACOB, MacCheckJacob([SDIR 'JACOB_F'],jacobian), end

% perturbed plasma current
if Mac.RunJ
%[JM1,JM2,JM3] = MacReadJPLASMA('../SHOTAnalCirc/JPLASMA_WALL');
[JM1,JM2,JM3] = MacReadJPLASMA([SDIR 'JPLASMA_KINK_Smooth']);
