...
%Mac.coilN  = [3.23 0.80 3.23 -0.80];     
             % D3D C-coils
%Mac.coilN  = [2.138 0.942 2.518 0.550];
             % KSTAR RMP Upper coils
Mac.coilN  = [2.138 -0.942 2.518 -0.550];
             % KSTAR RMP Lower coils
%Mac.coilN  = [2.482 0.550 2.482 -0.550];
             % KSTAR RMP Middle coils
%Mac.rw     = [0.9294];
...
afac  = 1e+0; phas=0;
%afac  = 1.6071e-1;  phas=0; %D3D184602
%afac  = 1.3452e-1;  phas=0; %D3D185604
%afac   = 4.9510e-2; phas=0.0; %D3D177040
Mac.ALPHA   = 1.0;                     
...
%Mac.VNORM  = Mac.VNORM*Mac.R0EXP*1e+3*2; %in [mm]
...
res_Vn = [max(abs(Vn(end,:))) mean(abs(Vn(end,:))) abs(Vn(end,Imid)) abs(Vn(end,Ixpt))]
FIN_RES_xinX = Vn(end,Ixpt); 
