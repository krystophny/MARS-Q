%Plot deltaJ_para and perform SVD analaysis
%for 3D Equilibrium database
 
%function MacMain
%% setup for the circular case

global Mac
global SDIR SDIR5 k_plot FEEDI KFEED
...
Mac.RunEQ = 1;
Mac.plot_EQ = 0;
if k_plot==1, Mac.plot_EQ=10; end
...
Mac.DivJ  = 0;
Mac.plot_DivJ = 0;
Mac.ReduceNs = 0;  %reduce radial points when output J on Gaussian points
...
Mac.plot_V  = 0;
Mac.plot_Vn = 0;
if k_plot==1, Mac.plot_VM=7; end
if k_plot==1, Mac.plot_Vn=1; end
...
Mac.coilN  = [2.184 1.012 2.394 0.504; 2.394 -0.504 2.184 -1.012; 3.23 0.80 3.23 -0.80]; 
              % D3D I+C-coils: [R1 Z1 R2 Z2;...]
...
Mac.kSVDsum   = 2;  %sum all SVD eigenvectors from 1 to the given number
Mac.kRAW      = 0;
Mac.kSVDreconst = 0; 
Mac.DEN  = 3.3952e+19;
Mac.TI   = 2.7507e+03;
...
if Mac.RunJ
[JM1,JM2,JM3,JMRE] = MacReadJPLASMA([SDIR5 'JPLASMA.OUT']);
...
MacGetJpara(R,Z,dRds,dZds,dRdchi,dZdchi,jacobian,J1,J2,J3,JRE,JM1,JM2,JM3);
...
[BM1,BM2,BM3] = MacReadBPLASMA([SDIR5 'BPLASMA.OUT']);
...
% perturbed velocity
if Mac.RunV
[VM1,VM2,VM3,DPSIDS,T,VM1N] = MacReadVPLASMA([SDIR5 'XPLASMA.OUT']);
...
[Y,IxptU] = max(Z(Mac.Ns1,:));
[Y,IxptL] = min(Z(Mac.Ns1,:));
RES_Vn   = [max(abs(Vn(end,:))) mean(abs(Vn(end,:))) abs(Vn(end,Imid)) abs(Vn(end,IxptL))];
CRIT_Xn  = [Vn(end,IxptU); Vn(end,IxptL)];
RES      = [real(CRIT_Xn(1)) imag(CRIT_Xn(1)) real(CRIT_Xn(2)) imag(CRIT_Xn(2))];
save([SDIR5 'CRIT_Xn'],'RES','-ascii')

II  = find(Mac.s(1:Mac.Ns1)>0.975);
se  = Mac.s(II);
VneU = Vn(II,IxptU);
VneL = Vn(II,IxptL);
CRIT_Xn_meanU  = sum(diff(se).*(VneU(1:end-1)+VneU(2:end)))/2/(se(end)-se(1));
CRIT_Xn_meanL  = sum(diff(se).*(VneL(1:end-1)+VneL(2:end)))/2/(se(end)-se(1));
RES      = [real(CRIT_Xn_meanU) imag(CRIT_Xn_meanU) real(CRIT_Xn_meanL) imag(CRIT_Xn_meanL)];
save([SDIR5 'CRIT_Xn_mean'],'RES','-ascii')
...
% perturbed pressure
if Mac.RunP
[PM1,PM2,PM3] = MacReadPPLASMA([SDIR5 'PPLASMA.OUT']);
end
