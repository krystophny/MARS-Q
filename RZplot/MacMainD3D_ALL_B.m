...
%Mac.coilN  = [2.9319    1.6550    2.9311   -2.0300];     
             % MAST EFCC
%Mac.rw     = [0.9294];
Mac.rw     = [1.0391];
%Mac.rw     = [1.0565];
...
%afac = 1.1341e-04; %RP-RWM, ITER-10MA
afac = 1.0;
Mac.ALPHA   = 1.0;                     
...
%SDIR = '../../YQLiu/FUSION/Machines/MAST_ADD/Matlab/';
%SDIR = '../D3D164362/Work/'; 
...
Mac.BNORM  = Mac.BNORM*1e+4;  %dimensionless
...
if Mac.RunEF==1
   %MacGetB1EF([SDIR '29151_surf_ef_n=1.dat'],R,Z,dRdchi,dZdchi);
   %MacGetB1EF([SDIR '29151_surf_efcc_n=1.dat'],R,Z,dRdchi,dZdchi);
   %MacGetB1EF([SDIR '30090_surf_ef_n=2.dat'],R,Z,dRdchi,dZdchi);
     MacGetB1EF([SDIR '30090_surf_rmp_n=2.dat'],R,Z,dRdchi,dZdchi);
   %MacGetB1EF([SDIR '28002_surf_ef_n=1_10738ESC.dat'],R,Z,dRdchi,dZdchi);
   %MacGetB1EF([SDIR '10738_surf_elm11_n=1.dat'],R,Z,dRdchi,dZdchi);
...
% perturbed velocity
if Mac.RunV
[VM1,VM2,VM3,DPSIDS,T,VMIN] = MacReadVPLASMA([SDIR 'XPLASMA.OUT']);
[V1,V2,V3,Vn] = MacGetV123(VM1,VM2,VM3,VMIN,R,Z,dRds,dZds,dRdchi,dZdchi,jacobian);
