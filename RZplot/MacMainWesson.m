...
Mac.coil   = [1.16 0.45 0.6; 1.16 -0.6 -0.45];     
           % coils position defined as [rc chi_1 chi_2]
Mac.resetCoil = 0;
Mac.coilN  = [4.98 1.0 4.98 -1.0; 3.77 1.22 4.07 0.70; 4.07 -0.70 3.77 -1.22; 3.05 1.84 2.33 1.84; 3.05 -1.84 2.33 -1.84];     
...
if Mac.RunEQ
[QEQ,TEQ,DPSIEQ] = MacReadPROFEQ([SDIR 'PROFEQ.OUT']);
end
...
fac   = mu0/Mac.R0EXP;
FEEDI = 1.0;
%Mac.BNORM  = Mac.BNORM*fac*I0EXP/FEEDI*1e+4;  %in Gauss
%Mac.VNORM  = Mac.BNORM*Mac.R0EXP/Mac.B0EXP/1e+4;
...
if Mac.checkJACOB, MacCheckJacob('JACOBIAN_fix',jacobian), end
...
if Mac.RunEQ>0 & Mac.plot_Jpara>0
   MacGetJpara(R,Z,dRds,dZds,dRdchi,dZdchi,jacobian,TEQ,DPSIEQ,J1,J2,J3);
end
if Mac.RunEQ>0 & Mac.plot_testStarEq>0
   [JparaM2,JparaM3]=MacTestStarEqLi(R,Z,dRds,dZds,dRdchi,dZdchi,jacobian,TEQ,DPSIEQ,J1,J2,J3);
end
...
dRds = dRds(1:Mac.Ns,:);
dZds = dZds(1:Mac.Ns,:);
dRdchi = dRdchi(1:Mac.Ns,:);
dZdchi = dZdchi(1:Mac.Ns,:);
jacobian = jacobian(1:Mac.Ns,:);
...
de = load('FAR_EQ.txt');
dr = [db(:,1) de(:,2:3) db(:,2)];
save FAR_RES dr -ascii
!rm FAR_BM.txt FAR_EQ.txt  
end

[BM1P,BM1A] = MacDeltapGetBM1P(BM1,BM2,BM3);
...
if Mac.plot_testStarEq>0
  mres = 2;
  nres = mres-Mac.Mm(1) + 1;
  II = find(Mac.s>0.9 & Mac.s<0.906); 
  sres = Mac.s(II);
  wres = Mac.omega(II);
  dres=Mac.dpsi(II);
  qres=mres+Mac.n*Mac.q(II);
  eres = Mac.Resieta(II);

  figure(10*Mac.plot_testStarEq+0)
  subplot(2,2,1), plot(sres,wres), hold on,
  subplot(2,2,2), plot(sres,dres), hold on,
  subplot(2,2,3), plot(sres,qres), hold on,
  subplot(2,2,4), plot(sres,eres), hold on,

  figure(10*Mac.plot_testStarEq+1)
  subplot(1,2,1), plot(sres,real(BM1(II,nres)),'r-',sres,imag(BM1(II,nres)),'b-'), hold on  
  subplot(1,2,2), plot(sres,real(wres.*BM1(II,nres)),'r-',sres,imag(wres.*BM1(II,nres)),'b-'), hold on  

  figure(10*Mac.plot_testStarEq+2)
  fres = dres.*qres.*wres*i;
  subplot(1,2,1), plot(sres,real(VM1(II,nres)),'r-',sres,imag(VM1(II,nres)),'b-'), hold on  
  subplot(1,2,2), plot(sres,real(fres.*VM1(II,nres)),'r-',sres,imag(fres.*VM1(II,nres)),'b-'), hold on  

  J23=JparaM2(II,nres)-mres/Mac.n*JparaM3(II,nres);
  figure(10*Mac.plot_testStarEq+3)
  subplot(1,2,1), plot(sres,real(J23),'r-',sres,imag(J23),'b-'), hold on  
  subplot(1,2,2), plot(sres,real(eres.*J23),'r-',sres,imag(eres.*J23),'b-'), hold on  
  
  figure(10*Mac.plot_testStarEq+4)
  plot(sres,real(fres.*VM1(II,nres)),'r-',sres,imag(fres.*VM1(II,nres)),'r--'), hold on  
  plot(sres,real(eres.*J23),'b-',sres,imag(eres.*J23),'b--'), hold on  

  RHS = fres.*VM1(II,nres) + eres.*J23;
  figure(10*Mac.plot_testStarEq+5)
  plot(sres,real(wres.*BM1(II,nres)),'r-',sres,imag(wres.*BM1(II,nres)),'r--'), hold on  
  plot(sres,real(RHS),'b-',sres,imag(RHS),'b--'), hold on  
end


% perturbed pressure
if Mac.RunP
[PM1,PM2,PM3] = MacReadPPLASMA('PPLASMA.OUT');
end
