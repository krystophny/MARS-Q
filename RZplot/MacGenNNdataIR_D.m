% generate NN database for 3D equilibria
% according to the input/output format defined in drawMemo.m 

SDIR1 = '/cscratch/liuy/WorkIR_D/Database/';
SDIRM = '/home/liuy/IRIS/mars-code/RZplot/'; 

DEV = {'MAST','DIII-D','AUG','ITER'};
SN  = '1234';
LN  = zeros(1,size(DEV,2));
kd1 = 1;
kSUV = 5;  %NN output with first kSUV modes

chi1 = linspace(-pi,pi,301);  %internal poloidal mesh for plasma boundary
s1   = linspace(0,1,20);      
s1   = s1.^(1./(2+4*s1));     
s1   = [0 s1(2)/2 s1(2:end)]; %s-mesh for NN input equilibrium profiles 
s2   = linspace(0,1,200);     
s2   = s2.^(1./(2+4*s2));     
s2   = [0 s2(2)/2 s2(2:end)]; %s-mesh for NN output U-vectors 

for kd=kd1:size(DEV,2)
    if kd==1 | kd==3, SC='UL';  end
    if kd==2,         SC='ULC'; end
    if kd==4,         SC='ULM'; end

    SDIR2 = [SDIR1 DEV{kd} '/'];
    cd(SDIR2)
    F  = split(ls);
    NF = length(F)-1
    cd(SDIRM)

    for kf=1:NF
        FF = F{kf};
        for kn=1:length(SN)
        for kc=1:length(SC)
            SDIR3 = [SDIR2 FF '/3D/n1/'];
            SDIR4 = [SDIR2 FF '/3D/n' SN(kn) '/'];
            SDIR5 = [SDIR4 SC(kc) '/'];

            %input:RZ: plasma boundary
            d  = load([SDIR3 'RMZM_F.OUT']);
            M  = round(d(1,1));
            N1 = round(d(1,2));
            N2 = round(d(1,3));
            N  = N1 + N2;
            RM = d(N+2:end,1) + d(N+2:end,2)*1i;
            ZM = d(N+2:end,3) + d(N+2:end,4)*1i;

            RM = reshape(RM,N,M);
            ZM = reshape(ZM,N,M);
            RM = RM(N1,:);
            ZM = ZM(N1,:);
            RM(2:end) = 2*RM(2:end);
            ZM(2:end) = 2*ZM(2:end);

            mm = transpose([0:1:M-1]); 
            expmchi = exp(mm*chi1*1i);
            R  = real(RM*expmchi);
            Z  = real(ZM*expmchi);
            
            Rmid = (max(R)+min(R))/2;
            R  = R/Rmid;
            Z  = Z/Rmid;
            Zmid = (max(Z)+min(Z))/2;

            t  = atan2(Z-Zmid,R-1);
            [t,I] = sort(t); R=R(I); Z=Z(I);
            [t,I] = unique(t); R=R(I); Z=Z(I);
            I1 = find(t>pi/2);
            I2 = find(t<-pi/2);
            tt = [t(I1)-2*pi t t(I2)+2*pi];
            RR = [R(I1) R R(I2)];
            ZZ = [Z(I1) Z Z(I2)];
            [tt,I] = unique(tt); RR=RR(I); ZZ=ZZ(I);
	    t1 = linspace(-pi,pi,33); t1 = t1(1:end-1);
            R1 = spline(tt,RR,t1);
            Z1 = spline(tt,ZZ,t1);
            
            RZ     = R1;
            RZ(9)  = Z1(9);
            RZ(25) = Z1(25);
            RZ     = [RZ Zmid];  %=Zmid/Rmid
 
            d  = load([SDIR3 'PROFEQ.OUT']);
            s  = d(:,1);

            %input:q: safety factor profile
            q = spline(s,d(:,2),s1(1:end-1));

            %input:P: pressure profile
            P = spline(s,d(:,4),s1);

            %input:rho: density profile
            rho = spline(s,d(:,5),s1);

            %input:w: toroidal rotation frequency profile
            w = spline(s,d(:,6),s1);

            %input:eta: resistivity profile
            eta = spline(s,d(:,7),s1(1:end-1));

            %input:dBvac: vacuum normal field along poloidal angle at s=1.01
            d  = load([SDIR5 'dBnormal.txt']);
            t  = d(:,1);
            y  = d(:,2) + d(:,3)*1i;

            [t,I] = sort(t);   y=y(I);
            [t,I] = unique(t); y=y(I);
            I1 = find(t>pi/2);
            I2 = find(t<-pi/2);
            tt = [t(I1)-2*pi; t; t(I2)+2*pi];
            yy = [y(I1); y; y(I2)];
            [tt,I] = unique(tt); yy=yy(I);
	    t1 = linspace(-pi,pi,101); t1 = t1(1:end-1);
            dBvac = spline(tt,yy,t1);

            %input:AI:all data
            AI = [RZ(:); q(:); P(:); rho(:); w(:); eta(:); real(dBvac(:)); imag(dBvac(:))];
            
            %output
            d  = load([SDIR4 'PROFEQ.OUT']);
            s  = d(:,1);
            dj = load([SDIR5 'SVD_Jpara5.txt']);
            dx = load([SDIR5 'SVD_X1U5.txt']);
            ND = round(size(dj,2)/2);
            JJ = [1:kSUV ND+[1:kSUV]];
            NS = length(s);

            %output:SJ,SX: singular values
            SJ = dj(2,1:kSUV);
            SX = dx(2,1:kSUV);
            
            %output:UJ,UX: U-vectors
            UJ = dj(3:NS+2,JJ);
            UX = dx(3:NS+2,JJ);

            UJN = zeros(length(s2),size(UJ,2));
            UXN = UJN;
            for k=1:size(UJ,2)
                UJN(:,k) = spline(s,UJ(:,k),s2);
                UXN(:,k) = spline(s,UX(:,k),s2);
            end
            UJ = UJN;
            UX = UXN;

            %output:VJ,VX: V-vectors
            VJ = dj(NS+3:end,JJ);
            VX = dx(NS+3:end,JJ);
                        
            %output:AOJ,AOX:all data
            AOJ = [SJ(:); UJ(:); VJ(:)];
            AOX = [SX(:); UX(:); VX(:)];

            %save all data into file for NN
            AJ = [length(AI); length(AOJ); length(RZ); length(P); length(dBvac); length(SJ); size(UJ,1); size(VJ,1); AI; AOJ];
            AX = [length(AI); length(AOX); length(RZ); length(P); length(dBvac); length(SJ); size(UX,1); size(VX,1); AI; AOX];
            save([SDIR5 'NNdata_Jpara.dat'],'AJ','-ascii') 
            save([SDIR5 'NNdata_X1U.dat'],'AX','-ascii') 
         end
         end
     end
end

