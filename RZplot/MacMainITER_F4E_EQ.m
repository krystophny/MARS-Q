...
%Mac.coilN  = [5715 7100 7471.5 7100; ...
%						 11297 3885.46 11297 -2948.3; ...
%                  5715 -7360 7471.5 -7360]/1000;
              % ITER EFCC: [R1 Z1 R2 Z2;...]
Mac.rw     = [1.0 1.315 1.577];
...
SDIR = '/home/yliu/F4E_2014/DataRes/10470/n=2/';
%SDIR = '/home/yliu/ITER/Scen4_V17/RmpOpt/n=1/';
...
% RZ coordinates
[RM,ZM] = MacReadRMZM([SDIR 'RMZM_F_GEOM']);
[R,Z] = MacGetRZ(RM,ZM);
...
% collect and output EQ data for Ivan
d = load([SDIR '../n=1/PROFEQ_01.OUT']);
s2 = d(:,1);
Q2 = d(:,2);
P2 = d(:,4)*Mac.B0EXP^2/(4.0e-7*pi);
F2 = d(:,13)*Mac.B0EXP*Mac.R0EXP;

Q = spline(s2,Q2,Mac.s(1:Mac.Ns1));
P = spline(s2,P2,Mac.s(1:Mac.Ns1));
F = spline(s2,F2,Mac.s(1:Mac.Ns1));

res = zeros(Mac.Ns1*Mac.Nchi,5);
for j=1:Mac.Ns1
for k=1:Mac.Nchi
     l = k+(j-1)*Mac.Nchi;
     res(l,1) = R(j,k)*Mac.R0EXP;
     res(l,2) = Z(j,k)*Mac.R0EXP;
     res(l,3) = P(j);
     res(l,4) = F(j);
     res(l,5) = Q(j);
end
...
save ITER_15MA_EQ_DATA res -ascii -double
DIM_ITER_15MA_EQ_DATA = [Mac.Ns1 Mac.Nchi 5]

res = [R(Mac.Ns1,:)' Z(Mac.Ns1,:)']*Mac.R0EXP;
save ITER_15MA_EQ_BOUND res -ascii -double
