...
| perturbative passive-operator ordering | native `IPERTURB=1` and imported `KPERTREAD=1` fields assemble passive `KPBKEY=1` blocks before torque; only imported B/X is restored | paired native/reload B/X and DWK equality plus matching nonzero `TORQUENTV.OUT` |
