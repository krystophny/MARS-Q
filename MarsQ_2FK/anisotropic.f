...
C        NOTE THAT THIS OPTION HOLDS ONLY FOR ENERGY-INDEPENDENT COLLISIONALITY
     ...
     &      RTMP,RTMP3)
         IF (OTRACE) CALL WRITEELLACTIONENERGY(JS,KGRID,KP,KOPT,
     &      KDPHI,L,RL,ZZLAMB,FNUMSOURCE,DENOMTRACE,FNUM,FTMP,
     &      RFAC2,OMEGAN0,RTMP,RTMP3)
...
C=========================================================
      SUBROUTINE WRITEELLACTIONENERGY(JS,KGRID,KP,KOPT,KDPHI,L,
     &   RL,ZZLAMB,FNUMSOURCE,DENOMTRACE,FNUM,FTMP,RFAC2,
     &   OMEGAN0,RTMP,RTMP3)
      USE GLOBALM
      USE KINETICM
      USE ANISOTROPICM
      ...
      IMPLICIT NONE
      INCLUDE 'compam.inc'
      INTEGER JS,KGRID,KP,KOPT,KDPHI,L,K,N,FID
...
C$OMP CRITICAL(ELL_ACTION_TRACE_WRITE)
     ...
     &     ACTION='WRITE')
      IF (.NOT.OEXIST) WRITE(FID,*)
     & '% JS G KP KOPT KDPHI L ENODE ELL LAMBDA ZEPKO ENERGY WEIGHT',
...
C     interval.  Both nodes therefore carry half of that interval width.
      DO K=1,2*(NEPK-1)
         ...
         WRITE(FID,1000) JS,KGRID,KP,KOPT,KDPHI,L,K,RL,ZZLAMB,
