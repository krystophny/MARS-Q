...
C=========================================================
     ...
     &           KTERM,KTRACEJOIN,K,KCHECK,KEXEC,KDPHI
      ...
      REAL*8     H1,H2,H3,H4,H5,H6,H7,FAC0,OMEGAEDPSI,ZZT,OMEGANRE
      COMPLEX*16 ZVIF,OMEGAN,OMEGAN0,FNUM0,ZTMP0,ZVIFREG
      COMPLEX*16 SUBTRACE(4)
     ...
     &    NINT(RL).EQ.KNTVELL) THEN
      ...
      ENDIF
      IF (OTRACE) THEN
         IF (JS_MAT.LT.1.OR.KPITCH.LT.1.OR.KPARTICLE.NE.0.OR.
     &       ICASE.NE.1.OR.KTERM.NE.1)
     &      STOP 'INVALID ACTION FACTOR JOIN IDENTITY'
      ENDIF
...
C     Emit both resonance normalizations.  The RTMP-small branch above is
C     an executed finite response with the unscaled denominator
C     OMEGAN+i*FTMP; omitting it makes a drift-zero cell look unexecuted.
      IF (OTRACE) CALL WRITEELLTRACEENERGY(JS,KGRID,KP,L,RL,
     &   ZZLAMB,FNUMSOURCE,DENOMTRACE,FNUM,FTMP,RFAC2,OMEGAN0,
     &   RTMP,RTMP3)
      IF (OTRACE) CALL WRITEELLACTIONENERGY(JS,JS_MAT,KGRID,KP,KOPT,
     &   KDPHI,L,KPITCH,KPARTICLE,ICASE,KTERM,RL,ZZLAMB,
     &   FNUMSOURCE,DENOMTRACE,FNUM,FTMP,RFAC2,OMEGAN0,RTMP,RTMP3)
  
      DO K=1,NEPK-1
         HEPK = EPK(K+1)-EPK(K)
         ZVIF = ZVIF + 0.5*HEPK*(FNUM(2*K-1)+FNUM(2*K))
         ZVIFREG = ZVIFREG + 0.5*HEPK*(FNUM(2*K-1)+FNUM(2*K))
      ENDDO

      IF (OTRACE) CALL WRITEELLTRACEPITCH(JS,KGRID,KP,L,RL,
     &                                    ZZLAMB,ZVIF)
      IF (OTRACE) CALL WRITEELLTRACERESPONSE(JS,JS_MAT,KGRID,KP,KOPT,
     &   KDPHI,L,KPITCH,KPARTICLE,ICASE,KTERM,RL,ZZLAMB,ZVIFREG,
     &   SUBTRACE,ZVIF)
...
C=========================================================
      SUBROUTINE WRITEELLACTIONENERGY(JS,JS_MAT,KGRID,KP,KOPT,KDPHI,L,
     &   KPITCH,KPARTICLE,ICASE,KTERM,RL,ZZLAMB,
     &   FNUMSOURCE,DENOMTRACE,FNUM,FTMP,RFAC2,
     &   OMEGAN0,RTMP,RTMP3)
      USE GLOBALM
      USE KINETICM
      USE ANISOTROPICM
     ...
     &        ICASE,KTERM,K,N,FID
      REAL*8 RL,ZZLAMB,RTMP,RTMP3,EWEIGHT
      COMPLEX*16 OMEGAN0
      COMPLEX*16 FNUMSOURCE(NEPK2),DENOMTRACE(NEPK2),FNUM(NEPK2)
      REAL*8 FTMP(NEPK2),RFAC2(NEPK2)
      LOGICAL OEXIST
...
C$OMP CRITICAL(ELL_TRACE_WRITE)
     ...
     &     ACTION='WRITE')
      IF (.NOT.OEXIST) THEN
         WRITE(FID,*) '% SCHEMA 2'
         WRITE(FID,*)
     & '% JS G KP KOPT KDPHI L ENODE JSMAT PITCH PARTICLE ICASE TERM',
     & ' SCALAR_SLOT ELL LAMBDA ZEPKO ENERGY WEIGHT',
     & ' SOURCE_RE SOURCE_IM DENOM_RE DENOM_IM INTEGRAND_RE',
     & ' INTEGRAND_IM COLLISION_CORR RFAC2 OMEGAN0_RE OMEGAN0_IM',
     & ' RTMP RTMP3 OMEGAB DRIFT OMEGAE NUEFF SLAM0'
      ENDIF
C     The executed energy integral is a two-point Gauss rule in each EPK
C     interval.  Both nodes therefore carry half of that interval width.
      DO K=1,2*(NEPK-1)
         N=(K+1)/2
         EWEIGHT=0.5*(EPK(N+1)-EPK(N))
         WRITE(FID,1000) JS,KGRID,KP,KOPT,KDPHI,L,K,JS_MAT,KPITCH,
     &      KPARTICLE,ICASE,KTERM,KDPHI+1,RL,ZZLAMB,
     &      ZEPKO(K),ZEPK(K),EWEIGHT,FNUMSOURCE(K),DENOMTRACE(K),
     &      FNUM(K),FTMP(K),RFAC2(K),OMEGAN0,RTMP,RTMP3,OMEGAB,
     &      DRIFT,OMEGAE0(JS,KGRID),NUEFF(JS,KGRID,KP),SLAM0(L,KP)
      ENDDO
      CLOSE(FID)
C$OMP END CRITICAL(ELL_TRACE_WRITE)
 1000 FORMAT(13I8,22(1X,E24.16))
      END SUBROUTINE WRITEELLACTIONENERGY
