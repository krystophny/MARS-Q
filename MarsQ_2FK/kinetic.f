...
C           THE SINGULAR COEFFICIENT IS PART OF THE SAME COMPLEX
C           RESPONSE AS THE REGULAR PITCH INTEGRAL, SO IT CARRIES THE
C           SAME RESPONSE-PART PROJECTION.  KIA_TRAP SUBTRACTS
C           LOG|LAM-LAM0|*SF0 AND KJPFILL ADDS IT BACK, SO PROJECTING
C           ONLY THE REGULAR PART LEAVES EACH LANE HOLDING THE WHOLE
C           SINGULAR TERM AND THE TWO LANES DOUBLE COUNT IT.
            CALL KELLRESPONSEPART(ZVI,RLM(L))
            SF0(L,KP,KOPT,1) = ZVI
            IF (INCDPHI.GT.0) THEN
