...
C     INTEGRATE I-FACTOR ON A DENSE LAMBDA MESH FOR RLM(L)=0
      IF (SUM(ABS(PSPECIES_NTD)).GT.0.AND.SLAMD0.GT.0.) THEN   
         CALL WRITE_KI0_COVERAGE(JS,KGRID,SLAMD0,1)
         CALL KI0(JS,KGRID,0)
         IF (IFOWT.EQ.1) THEN
            CALL KI0(JS,KGRID,2)
            CALL KI0(JS,KGRID,3)
         ENDIF
      ELSE
C        REACHED, NOT INTEGRATED.  RECORDING THIS IS WHAT LETS A READER
C        TELL "NO PRECESSION ROOT HERE" FROM "NEVER REACHED":
C        WITHOUT IT BOTH CASES LOOK IDENTICAL, AS A MISSING RECORD.
         CALL WRITE_KI0_COVERAGE(JS,KGRID,SLAMD0,0)
      ENDIF
...
C        requested (JS,KGRID,ELL) row from ELL_TRACE.REQUEST and are read-only
C        with respect to the production arrays.
         CALL WRITEKJPTRAPPEDORBIT(JS,JS_MAT,KGRID,LAM)
...
C=======================================================================
     ...
     &           K,M,KP,J,L,N,M00,J00,J2M,J1M,J1P,J2P,KCALL
     ...
     & KPITCH,KPARTICLE,ICASE,LAM,LAMH,KCALL)
      DO K=1,MSMAX
      DO M=1,MSMAX
     ...
     &                                  *H3
      ...
      IF (OTRACE) CALL WRITEKJPFILLCELL(JS,JS_MAT,KGRID,KCALL,KPITCH,
     &   KPARTICLE,ICASE,K,M,LAM,LAMH,
     &   SUM(SUM(X1PARA,1),1),SUM(SUM(X1PERP,1),1),
     &   SUM(SUM(X1DPHI,1),1),SUM(SUM(X2PARA,1),1),
     &   SUM(SUM(X2PERP,1),1),SUM(SUM(X2DPHI,1),1),
     &   SUM(SUM(Q1PARA,1),1),SUM(SUM(Q1PERP,1),1),
     &   SUM(SUM(Q1DPHI,1),1),SUM(SUM(Q2PARA,1),1),
     &   SUM(SUM(Q2PERP,1),1),SUM(SUM(Q2DPHI,1),1),
     &   SUM(SUM(Q3PARA,1),1),SUM(SUM(Q3PERP,1),1),
     &   SUM(SUM(Q3DPHI,1),1),SUM(SUM(DPPARA,1),1),
     &   SUM(SUM(DPPERP,1),1),SUM(SUM(DPDPHI,1),1))
...
C=======================================================================
     ...
     & KPARTICLE,ICASE,RLAM,RLAMH,KCALL)
      ...
      INTEGER JS,JS_MAT,KGRID,KPITCH,KPARTICLE,ICASE,KP,R,L,FID,KCALL
...
C=======================================================================
      ...
      COMPLEX*16 VVI0BASE(4,NSPECIES0),VVI0LANDAU(4,NSPECIES0)
      COMPLEX*16 VVI0DRIFT(4,NSPECIES0),VVI0DRIFTCAND(4,NSPECIES0)
     ...
     &           LAMNH(2*(NN-1)),LAMX(NN)
      ...
      INTEGER KCHECK, NUMSIG, DRIFTAPPLIED(NSPECIES0)
      KCHECK=0
      NUMSIG=3
      VVI0BASE = 0.
      VVI0LANDAU = 0.
...
C        MISSING ONE, AND THE RECORDED WFUN SHOWS WHY VI0 IS ZERO HERE.
         IF (KOPT.EQ.0) THEN
            CALL WRITE_KI0_FACTOR(JS,KGRID,SLAMD0)
            DO KP=1,NSPECIES
               IF (ABS(PSPECIES_NTD(KP)).GT.0.)
     &            CALL WRITEELLTRACEKI0(JS,KGRID,KP,0,SLAMD0,DPRM,
     &            WFUN(JS,KGRID),VVI0BASE(:,KP),VVI0LANDAU(:,KP),
     &            VVI0DRIFT(:,KP),VVI0DRIFTCAND(:,KP),VI0(:,KP))
            ENDDO
         ENDIF
         RETURN
      ENDIF
...
C     NUMERICAL TREATMENT OF OMEGAD SINGULARITY FOR ANY SPECIES
     ...
     &              +OMEGAST(JS,KGRID,KP)+CI*NUEFF(JS,KGRID,KP)/RNTOR)
            VVI0DRIFTCAND(1,KP)=CTMP
            WRITE(*,*)'KI0 NUMSIG=0 RESIDU=',CTMP,
     $                'JS=',JS,'KGRID=',KGRID,'KP=',KP
         ELSE
            VVI0DRIFTCAND(1,KP)=-CI*RTMP*(OMEGASN(JS,KGRID,KP)
     &              +OMEGAST(JS,KGRID,KP)+CI*NUEFF(JS,KGRID,KP)/RNTOR)
            VVI0DRIFT(1,KP)=VVI0DRIFTCAND(1,KP)
            DRIFTAPPLIED(KP)=1
     ...
     &              +OMEGAST(JS,KGRID,KP)+CI*NUEFF(JS,KGRID,KP)/RNTOR)
      ...
      IF (KOPT.EQ.0) THEN
         DO KP=1,NSPECIES
            IF (ABS(PSPECIES_NTD(KP)).GT.0.)
     &         CALL WRITEELLTRACEKI0(JS,KGRID,KP,DRIFTAPPLIED(KP),SLAMD0,
     &         DPRM,WFUN(JS,KGRID),VVI0BASE(:,KP),VVI0LANDAU(:,KP),
     &         VVI0DRIFT(:,KP),VVI0DRIFTCAND(:,KP),VI0(:,KP))
         ENDDO
      ENDIF
...
C=======================================================================
C WRITE_KI0_COVERAGE
C
C RECORD THAT THE KI0 CALL SITE WAS REACHED FOR (JS,KGRID), AND WHETHER
C THE PRECESSION-RESONANCE INTEGRAL WAS TAKEN THERE.
C
C KI0 IS CALLED ONLY UNDER SUM(ABS(PSPECIES_NTD)) > 0 .AND. SLAMD0 > 0,
C SO A SURFACE WITHOUT A PRECESSION ROOT WRITES NO FACTOR RECORD AT ALL.
C SO DOES A SURFACE THE SWEEP NEVER GOT TO.  THE FACTOR FILE ALONE
C THEREFORE CANNOT SAY WHICH HAPPENED, AND A COVERAGE COUNT TAKEN FROM
C IT ALONE WOULD BE READ AS PHYSICS WHEN IT MAY BE BOOKKEEPING.
C
C THIS FILE CLOSES THAT: A SURFACE ABSENT FROM IT WAS NEVER REACHED,
C CALLED=0 MEANS REACHED WITH NO PRECESSION ROOT, CALLED=1 MEANS THE
C INTEGRAL WAS TAKEN AND THE FACTOR FILE HOLDS ITS RECORDS.
C
C DIAGNOSTIC ONLY.  NOTHING THE SOLVE READS IS TOUCHED.
C=======================================================================
      SUBROUTINE WRITE_KI0_COVERAGE(JS,KGRID,LAM0,CALLED)
...
C$OMP CRITICAL(WRITE_KI0_FACTOR_FILE)
      INQUIRE(FILE='KI0_COVERAGE.OUT',EXIST=EXISTS)
      FID = ASSIGNFREEFILEUNIT()
      IF (EXISTS) THEN
         OPEN(UNIT=FID,FILE='KI0_COVERAGE.OUT',STATUS='OLD',
     &        POSITION='APPEND',ACTION='WRITE')
      ELSE
         OPEN(UNIT=FID,FILE='KI0_COVERAGE.OUT',STATUS='REPLACE',
     &        ACTION='WRITE')
         WRITE(FID,'(A)') '% MARS-K precession-resonance call coverage'
         WRITE(FID,'(A)') '% JS KGRID CALLED SLAMD0 SUM|PSPECIES_NTD|'
         WRITE(FID,'(A)') '% CALLED=1: integrated;'//
     &      ' its records are in KI0_FACTOR.OUT'
         WRITE(FID,'(A)') '% CALLED=0: reached, no precession root here'
         WRITE(FID,'(A)') '% absent: the surface was never reached'
         WRITE(FID,'(A)') '% not in surface order; each record is keyed'
      ENDIF
      WRITE(FID,131) JS,KGRID,CALLED,LAM0,SUM(ABS(PSPECIES_NTD))
      FLUSH(FID)
      CLOSE(FID)
C$OMP END CRITICAL(WRITE_KI0_FACTOR_FILE)

 131  FORMAT(3(1X,I6),2(1X,E24.16))
