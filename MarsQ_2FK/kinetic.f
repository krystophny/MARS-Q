...
C     SPLINE DRIFT(LAM)
      CALL WRITE_KI0_ENDPOINTS(JS,KGRID,LAMM(1),
...
C=======================================================================
C WRITE THE TWO DRIFT ENDPOINTS CONSUMED BY THE KI0 SPLINE
C
C KI0'S DENSE PITCH MESH IS NOT THE ORDINARY KIA_TRAP PITCH MESH.
C RECORD THE ENDPOINTS IMMEDIATELY BEFORE SPLINE1D CONSUMES THEM.
C ONE ROW PER ACTIVE SPECIES CARRIES THE SIGNED SPECIES MULTIPLIER,
C CHARGE RATIO AND B0K PITCH-MEASURE NORMALIZATION.
C
C DIAGNOSTIC ONLY.  NOTHING THE SOLVE READS IS TOUCHED.
C=======================================================================
      SUBROUTINE WRITE_KI0_ENDPOINTS(JS,KGRID,LAMLO,LAMHI)
...
C$OMP CRITICAL(WRITE_KI0_FACTOR_FILE)
      ...
      IF (EXISTS) THEN
         OPEN(UNIT=FID,FILE='KI0_ENDPOINT.OUT',STATUS='OLD',
     &        POSITION='APPEND',ACTION='WRITE')
      ELSE
         OPEN(UNIT=FID,FILE='KI0_ENDPOINT.OUT',STATUS='REPLACE',
     &        ACTION='WRITE')
         WRITE(FID,'(A)') '% MARS-K KI0 spline endpoint drift values'
         WRITE(FID,'(A)') '% JS KGRID KP NLAMK0 B0K charge_ratio ' //
     &      'PSPECIES_NTD lambda_lo drift_lo lambda_hi drift_hi'
         WRITE(FID,'(A)') '% charge_ratio=ESPECIES_Z(KP)/' //
     &      'ESPECIES_Z(1); pitch weights contain 1/(B0K*sqrt(pi))'
         WRITE(FID,'(A)') '% not in surface order; each record is keyed'
      ENDIF
      DO KP=1,NSPECIES
         IF (ABS(PSPECIES_NTD(KP)).LE.0.) CYCLE
         ZRATIO = ESPECIES_Z(KP)/ESPECIES_Z(1)
         WRITE(FID,130) JS,KGRID,KP,NLAMK0(JS,KGRID),B0K,ZRATIO,
     &      PSPECIES_NTD(KP),LAMLO,ZOMEGADT(JS,1,KGRID),LAMHI,
     &      ZOMEGADT(JS,2*NLAMK0(JS,KGRID),KGRID)
      ENDDO
      FLUSH(FID)
      CLOSE(FID)
C$OMP END CRITICAL(WRITE_KI0_FACTOR_FILE)

 130  FORMAT(4(1X,I6),7(1X,E24.16))
