...
C     COMPUTE EQUILIBRIUM QUANTITIES ON RCHI GRID
      ...
      IF (KELLTRACEACTIVE(JS,KGRID))
     &   CALL WRITEBOUNCEMETRICTRACE(JS,KGRID)
...
C=======================================================================
C DEFAULT-OFF TRACE OF THE EXACT EQUILIBRIUM FACTOR USED BY KBTIME.     =
C THE EXISTING ELL_M1_TRACE.REQUEST SELECTS SURFACES.  THIS WRITES ONE  =
C SMALL RECORD PER CHI NODE, BEFORE ANY PITCH OR ENERGY INTEGRATION.    =
C=======================================================================
      SUBROUTINE WRITEBOUNCEMETRICTRACE(JS,KGRID)
      ...
      INTEGER JS,KGRID,J,FID
      REAL*8 ZJAC,ZDPSI,ZB,ZH,ZJBPSI
      CHARACTER*64 PATH

      WRITE(PATH,'("ELL_M1_TRACE_JS",I4.4,"_G",I1,
     &             "_BOUNCE_METRIC.OUT")') JS,KGRID
C$OMP CRITICAL(ELL_TRACE_WRITE)
      FID=ASSIGNFREEFILEUNIT()
      OPEN(FID,FILE=PATH,STATUS='REPLACE',ACTION='WRITE')
      WRITE(FID,*)
     & '% JS G J CHI B H JAC DPSIDS JAC_B_OVER_DPSIDS RBT'
      DO J=1,NCHI
         IF (KGRID.EQ.1) THEN
            ZJAC=RJA(JS,J)
            ZDPSI=DPSIDS(JS)
         ...
         ZB=BK(JS,J,KGRID)
         ZH=HK(JS,J,KGRID)
         ZJBPSI=ZJAC*ZB/ZDPSI
         WRITE(FID,1000) JS,KGRID,J,RCHI(J),ZB,ZH,ZJAC,ZDPSI,
     &                   ZJBPSI,RBT(J)
      ENDDO
      CLOSE(FID)
C$OMP END CRITICAL(ELL_TRACE_WRITE)
 1000 FORMAT(3I8,7(1X,E24.16))
      END SUBROUTINE WRITEBOUNCEMETRICTRACE
...
C     COMPUTATION OF PPERP AND PPARA  
     ...
     &        IDRIVE
      LOGICAL ODIRECT,OBREAKDOWN,ODRIVELEDGER,OKELEDGER,
     &        OCACHEFINITE
