...
C     NOTE THAT OMEGAS*N INVOLVES DRHO/DPSI=(DRHO/DS)/(DPSI/DS)      
     ...
     &                 (1.-ETASTE)*DLNRHOM(J)*PEQM(J)/DPSIDSM(J))
      ...
      J = NRP1
      OMEGASI(J) = OMEGASI(J-1)
C     OMEGASE(NRP1) HAD NO EDGE ASSIGNMENT.  THE LOOP ABOVE FILLS 2..NR
C     AND THE AXIS BLOCK FILLS 1, SO THE BOUNDARY ELEMENT WAS AN
C     UNINITIALIZED READ: IT IS PRINTED IN PROFEQ.OUT COLUMN 15 AND IT
C     ENTERS DOMEGASEM(NR) AND SHIFTBC(NRP1).  ITS ION COUNTERPART
C     OMEGASI ALWAYS HAD THIS COPY; OMEGASE NOW MATCHES IT.
      OMEGASE(J) = OMEGASE(J-1)
      DLNRHO(J)  = DLNRHO(J-1)
      OMEGASIM(J)= 0.
      OMEGASEM(J)= 0.
