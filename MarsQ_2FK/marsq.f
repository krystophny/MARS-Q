...
C     ENERGY-CONTRACTION BLOCKS BEFORE OUTPUT FOR BOTH A NATIVE RESPONSE AND
C     AN IMPORTED MARS-F FIELD.  KPERTREAD ALSO SUPPORTS IPERTURB=2, SO RETAIN
C     ITS EXISTING PASSIVE ASSEMBLY.  ONLY AN IMPORTED FIELD NEEDS B/X RESTORED.
      IF (KNTV.EQ.21.AND.INCKIN.GT.0.AND.
     &   (KPERTREAD.EQ.1.OR.IPERTURB.EQ.1)) THEN
