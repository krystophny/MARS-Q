...
C     RESET EVERY TIME
     ...
     &   STOP 'KPROFTAUTH=1 REQUIRES NPROFIE=4 AND 2 MAXWELLIANS'
      ...
      IF (ABS(NTVNUMEREFAC-1.0).GT.1.0E-12.AND.
     &    ABS(NTVNUMEREFAC+1.0).GT.1.0E-12) GOTO 326
      IF (NTVNUMEREFAC.LT.0.0.AND.
     &   (KFASTRUN.NE.0.OR.IFOW.NE.0)) GOTO 327
...
C     DERIVED QUANTITIES
 ...
 326  WRITE(*,*) ' ERROR AT KINETIC INPUT'
      WRITE(*,*) ' NTVNUMEREFAC MUST BE +1 OR -1'
      IGO = -1
      RETURN

 327  WRITE(*,*) ' ERROR AT KINETIC INPUT'
      WRITE(*,*) ' NTVNUMEREFAC=-1 REQUIRES KFASTRUN=0, IFOW=0'
      IGO = -1
      RETURN
