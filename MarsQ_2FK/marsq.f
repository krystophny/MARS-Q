...
C     ENERGY-CONTRACTION BLOCKS EVEN THOUGH THE IMPORTED MARS-F FIELD MUST
C     NOT FEED BACK ONTO THE FLUID RESPONSE.  REASSEMBLE ONLY THE MISSING
C     BLOCKS, THEN RESTORE THE VALIDATED EXTERNAL B/X ARRAYS BEFORE OUTPUT.
      IF (KPERTREAD.EQ.1.AND.KNTV.EQ.21.AND.INCKIN.GT.0) THEN
...
C     CACHED PASSIVE OPERATOR USED TO CONTRACT A FROZEN EXTERNAL FIELD.
      IF (.NOT.(KDWKREAD.EQ.1.AND.KJPKEY.EQ.0))
