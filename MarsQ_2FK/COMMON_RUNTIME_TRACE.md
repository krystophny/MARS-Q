...
`KG`/`KH`; `endpoint_flag=-1/+1` rows carry the analytic lower/upper singular
add-backs.  `g_normalization` and `h_normalization` are the native factors
that must be applied after the quadrature sum.
