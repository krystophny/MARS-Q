...
C     THESE INCFEED OPTIONS ASSEMBLE THEIR SENSOR AND COIL MATRICES ON
C     A FIELD WHOSE TOROIDAL FUNCTION IS ITS EDGE VALUE, SO T=1 IS
C     WANTED BY FSUBM/GSUBM IN CALCFEED.  THE PROBLEM IS THAT THE
C     OVERRIDE IS GLOBAL AND PERMANENT: FEEDCTRL RUNS BEFORE KJP, SO
C     EVERY LATER USE OF T, INCLUDING THE KINETIC |B| AND ITS RADIAL
C     DERIVATIVE, ALSO SEES IT, AND THE KINETIC MODULE THEN WORKS ON
C     THE VACUUM TOROIDAL FIELD INSTEAD OF THE EQUILIBRIUM F(S).
C
C     KEEPTFUN=0 (DEFAULT) IS THE OLD BEHAVIOUR EXACTLY: T=1 STAYS.
C     KEEPTFUN=1 STILL APPLIES T=1 HERE, SO THE FEEDBACK ASSEMBLY IS
C     UNCHANGED, BUT SAVES F(S) AND ASKS MARSQ TO PUT IT BACK ONCE THE
C     ASSEMBLY IS DONE AND BEFORE KJP.  THAT ISOLATES THE KINETIC
C     CORRECTION INSTEAD OF ALSO MOVING THE FLUID RESPONSE.
      KTREST = 0
      IF (INCFEED.EQ.2.OR.INCFEED.EQ.4.OR.INCFEED.EQ.6.OR.
     *    INCFEED.EQ.10.OR.INCFEED.EQ.9.OR.INCFEED.EQ.12.OR.
     *    INCFEED.EQ.18.OR.INCFEED.EQ.22) THEN
      IF (KEEPTFUN.NE.0) THEN
         IF (.NOT.ALLOCATED(TSAVE))  ALLOCATE(TSAVE(NRP1))
         IF (.NOT.ALLOCATED(TMSAVE)) ALLOCATE(TMSAVE(NR))
         DO J=1,NRP1
            TSAVE(J)  = T(J)
         ENDDO
         DO J=1,NR
      ...
      DO J=1,NR
         TM(J) = 1.0    
      ENDDO
      IF (INCKIN.GT.0.AND.KEEPTFUN.EQ.0) THEN
